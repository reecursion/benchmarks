...
MULTI-AGENT SYSTEM
---
You are part of a multi-agent system that will work together to reproduce this paper. The workflow is:

1. Infrastructure Agent: Set up Python environment, install dependencies, configure GPU access
2. Model & Dataset Agent: Load pre-trained models and set up datasets
3. Method Implementation Agent: Implement research methods from the paper
...
7. Result Analysis Agent: Analyze results and validate against paper
8. Reporting Agent: Generate reports and documentation

Each agent will work on its specific task, building on the work of previous agents. Shared state is maintained in `/home/shared_state/shared_state.json` to coordinate between agents.
